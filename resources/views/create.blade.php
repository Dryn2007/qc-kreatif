...
<body
    class="bg-slate-950 text-slate-300 antialiased font-sans min-h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-900 via-[#0f172a] to-black py-10 px-4 sm:px-6">

    <div class="max-w-3xl mx-auto w-full relative">
        <!-- Glow background effects -->
        <div class="absolute -top-10 -left-10 w-72 h-72 bg-indigo-600/20 rounded-full blur-3xl pointer-events-none">
        </div>
        <div
            class="absolute -bottom-10 -right-10 w-72 h-72 bg-fuchsia-600/20 rounded-full blur-3xl pointer-events-none">
        </div>

        <div class="glass-card relative z-10 rounded-3xl shadow-2xl border border-slate-800 overflow-hidden">

            <div
                class="bg-slate-900/80 border-b border-slate-800 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-4">
                <div class="text-center md:text-left">
                    <h2 class="text-3xl font-extrabold text-white">✨ Jadwal <span
                            class="bg-gradient-to-r from-indigo-400 to-fuchsia-400 gradient-text">Baru</span></h2>
                    <p class="text-slate-400 text-sm font-medium mt-1">Tambahkan task ke board creative QC.</p>
                </div>
                <a href="{{ route('home') }}"
                    class="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all border border-slate-700 shadow-lg">
                    ← Kembali
                </a>
            </div>
            ...
            <div class="p-6 md:p-8">
                @if(session('success'))
                    <div
                        class="bg-emerald-900/30 border border-emerald-500/30 text-emerald-400 px-5 py-4 rounded-xl mb-8 flex items-center gap-3 font-semibold">
                ...
                <form action="{{ route('jadwal.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Nama Klien -->
                        <div>
                            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">🏷️ Nama
                                Klien</label>
                            <div class="relative">
                                <select name="klien" required
                                    class="w-full bg-slate-800/60 text-slate-200 border border-slate-700 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all font-bold appearance-none shadow-inner cursor-pointer relative z-10">
                                ...
                                <div
                                    class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-slate-400 z-20">
                        ...
                        <div>
                            ...
                            <input type="text" name="pilar_konten" required
                                ...
                                placeholder="Contoh: Story Telling, Q&A...">
                        </div>
